...
def main():
    ...
    turing = predict_target(model, 'portraits/alan_turing.png').read()
    col1.image(turing, use_column_width='always', caption='Alan Turing')
    einstein = predict_target(model, 'portraits/albert_einstein.png').read()
    col2.image(einstein, use_column_width='always', caption='Albert Einstein')
    tesla = predict_target(model, 'portraits/nikola_tesla.png').read()
